 ...
 *
 * {{{
 * $ java -cp target/kafka-streams-examples-5.4.1-standalone.jar io.confluent.examples.streams.MapFunctionScalaExample
 * }}}
 *
...
object MapFunctionScalaExample extends App {
  ...
  val config: Properties = {
    ...
    val bootstrapServers = if (args.length > 0) args(0) else "192.168.56.101:29094"
  ...
  val keyAndUppercased: KStream[String, String] = textLines.map((key, value) => (new String(key,"UTF-8").toUpperCase(), value.toUpperCase()))

  // Write the results(original key and uppercased value) to a new Kafka topic "KeyAndUppercasedTopic".
  keyAndUppercased.to("KeyAndUppercasedTopic")
